   ...
   (let [out (create-buffer)
     ...
     (generator 
       ...
       (let [res (if (not== last-q qval)
             ...
             new-z2 (+ (* G bp) lp)]
         (case mode
           0 (aset out int-indx lp) 
           1 (aset out int-indx hp)
           2 (aset out int-indx bp)
           3 (aset out int-indx (* 2.0 (* res bp))) 
           4 (aset out int-indx (- asig (* 2.0 (* res bp))))
           5 (aset out int-indx (- asig (* 4.0 (* res bp)))) 
           6 (- lp hp))
